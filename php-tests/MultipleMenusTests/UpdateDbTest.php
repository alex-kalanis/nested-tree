...
class UpdateDbTest extends AbstractMultipleMenusTests
{
    ...
    public function testIsNodeParentInItsChildren() : void
    {
        ...
        $categoryCondition->query = '`menu_id` = :menu_id';
        $categoryCondition->bindValues = [':menu_id' => 1];
        $categoryOption->additionalColumns = ['node.`menu_id`'];
        $categoryOption->where = $categoryCondition;
        $this->nestedSet->rebuild($categoryOption);

        $categoryCondition->query = 'node.`menu_id` = :menu_id';
        ...
        $categoryCondition->bindValues = [':menu_id' => 2];
        $this->nestedSet->rebuild($categoryOption);
    ...
    public function testChangeNodeParent() : void
    {
        ...
        // old group
        $where->bindValues = [':my_id' => 9, ':menu_id' => 1];
        $nodes = $this->nestedSet->listNodesFlatten($options);
        ...
        // new group
        $where->bindValues = [':my_id' => 16, ':menu_id' => 1];
        $nodes = $this->nestedSet->listNodesFlatten($options);
    ...
    public function testMoveNoEntry() : void
    {
        ...
        $options->additionalColumns = ['child.`menu_id`'];
        $where = new Support\Conditions();
        $where->query = 'node.`menu_id` = :menu_id';
        $where->bindValues = [':menu_id' => 1];
        $options->where = $where;
    ...
    public function testMoveNoConditionsMet() : void
    {
        ...
        $options->additionalColumns = ['child.`menu_id`'];
        $where = new Support\Conditions();
        $where->query = 'child.`menu_id` = :menu_id';
        $where->bindValues = [':menu_id' => 1];
        $options->where = $where;
    ...
    public function testUpdateData() : void
    {
        ...
        $this->assertFalse($this->nestedSet->update($node, $categoryTypedOption)); // no name can be set here

        $row = $this->getRow($this->database, $this->settings, 13);

        // updated
        $this->assertEquals(13, $row[$this->settings->idColumnName]);
        $this->assertEquals(9, $row[$this->settings->parentIdColumnName]);
        $this->assertEquals(2, $row[$this->settings->positionColumnName]);
    }
    ...
    public function testMoveWithoutChildren() : void
    {
        ...
        $node = next($nodes->items);
        $this->assertEquals(18, $node->id);
        $this->assertEquals(25, $node->left);
    ...
    protected function getOptions(int $menuId) : Support\Options
    {
        ...
        $categoryCondition = new Support\Conditions();
        $categoryCondition->query = '`menu_id` = :menu_id';
        $categoryCondition->bindValues = [':menu_id' => $menuId];
